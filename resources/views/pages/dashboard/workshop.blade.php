...
<div class="w-full min-h-screen flex flex-row bg-gradient-to-br from-blue-50 to-blue-100 text-gray-800">
    @include('layouts.Dashboard.sidebarkiri', [], ['class' => 'w-64 flex-shrink-0'])

    <div class="flex-1 p-4 md:p-8 ml-0 md:ml-64 transition-all duration-300 max-w-full overflow-hidden">
        <!-- Header Section -->
        <div class="mb-8 animate-fade-in">
            <h1 class="text-2xl md:text-4xl font-bold text-blue-700 mb-2">
                Manajemen Workshop & Event
            </h1>
            <p class="text-gray-600">Kelola workshop dan event pengembangan diri dengan mudah</p>
        </div>

        <!-- Notifications - Fixed positioning and overflow -->
        <div class="fixed top-4 right-4 z-50 max-w-sm w-full">
            @if(session('success'))
            <div class="bg-green-500 text-white px-4 py-3 rounded-xl mb-4 shadow-lg animate-slide-down">
            ...
            @if(session('error'))
            <div class="bg-red-500 text-white px-4 py-3 rounded-xl mb-4 shadow-lg animate-slide-down">
                <div class="flex items-center">
                    <svg class="w-5 h-5 mr-3 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div class="mb-8 animate-fade-in-up">
            <button onclick="openForm('add')" class="group relative bg-blue-600 hover:bg-blue-700 text-white px-6 md:px-8 py-3 md:py-4 rounded-xl font-semibold shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300 flex items-center space-x-3">
                ...
                <span class="text-sm md:text-base">Tambah Event/Workshop Baru</span>
            </button>
        </div>

        <!-- Popup Form -->
        <div id="popupForm" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden opacity-0 transition-opacity duration-300 p-4">
            <div id="popupContent" class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] relative transform scale-95 transition-transform duration-300 flex flex-col">
                <!-- Header -->
                <div class="bg-blue-600 text-white p-4 md:p-6 rounded-t-2xl flex-shrink-0">
                    <div class="flex justify-between items-center">
                        <h2 id="popupTitle" class="text-xl md:text-2xl font-bold">Tambah Event/Workshop</h2>
                ...
                <div class="p-4 md:p-8 overflow-y-auto flex-1">
                    <form id="mainForm" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div class="flex flex-col sm:flex-row gap-4 pt-6">
                            ...
                            <button type="button" onclick="closeForm()" class="flex-1 border border-gray-300 text-gray-700 hover:bg-gray-50 px-6 py-3 rounded-xl font-semibold flex items-center justify-center space-x-2 transition-all duration-200">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                                </svg>
                                <span>Batal</span>
                ...
                <div class="min-w-full inline-block align-middle">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-blue-50">
                            <tr>
                                <th class="px-3 md:px-6 py-3 md:py-4 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider">#</th>
                                <th class="px-3 md:px-6 py-3 md:py-4 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider">Gambar</th>
                                <th class="px-3 md:px-6 py-3 md:py-4 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider min-w-[200px]">Judul</th>
                                <th class="px-3 md:px-6 py-3 md:py-4 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider min-w-[120px]">Tanggal Mulai</th>
                                <th class="px-3 md:px-6 py-3 md:py-4 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider min-w-[140px]">Akhir Pendaftaran</th>
                                <th class="px-3 md:px-6 py-3 md:py-4 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider min-w-[120px]">Penyelenggara</th>
                                <th class="px-3 md:px-6 py-3 md:py-4 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider">Harga</th>
                                <th class="px-3 md:px-6 py-3 md:py-4 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider">Maks Peserta</th>
                                <th class="px-3 md:px-6 py-3 md:py-4 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider">Status</th>
                                <th class="px-3 md:px-6 py-3 md:py-4 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider min-w-[200px]">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-blue-50 transition-colors duration-200">
                                ...
                                <td class="px-3 md:px-6 py-4 whitespace-nowrap">
                                    <img src="{{ $workshop->foto ? asset('images/events/' . $workshop->foto) : 'https://via.placeholder.com/60x40' }}" alt="Event Image" class="w-12 h-8 md:w-16 md:h-10 object-cover rounded">
                                </td>
                                <td class="px-3 md:px-6 py-4">
                                    <div class="text-sm font-semibold text-gray-900">{{ $workshop->judul_event }}</div>
                                    <div class="text-xs text-gray-500">{{ Str::limit($workshop->deskripsi_event, 50) }}</div>
                                </td>
                                ...
                                <td class="px-3 md:px-6 py-4 text-sm text-gray-900">{{ $workshop->dilaksanakan_oleh }}</td>
                                <td class="px-3 md:px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                ...
                                <td class="px-3 md:px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $workshop->maksimal_peserta ?: 'Unlimited' }}</td>
                                <td class="px-3 md:px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-3 md:px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex flex-col sm:flex-row gap-2">
                                        <button type="button" class="w-full sm:w-auto bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded-lg transition-colors duration-200 flex items-center justify-center space-x-1 text-xs" onclick='openForm("detail", @json($workshop))'>
                                        ...
                                        <button type="button" class="w-full sm:w-auto bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-2 rounded-lg transition-colors duration-200 flex items-center justify-center space-x-1 text-xs" onclick='openForm("edit", @json($workshop))'>
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                            </svg>
                                            <span>Edit</span>
                                        </button>
                                        <form action="{{ route('dashboard.workshop.destroy', $workshop->id) }}" method="POST" onsubmit="return confirm('Yakin hapus workshop ini?')" class="w-full sm:w-auto">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="w-full bg-red-500 hover:bg-red-600 text-white px-2 py-1.5 rounded-md transition-colors duration-200 flex items-center justify-center space-x-1 text-xs">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<style>
    ...
    .fixed.top-4.right-4 > div {
        animation: slide-down 0.4s ease-out, fadeOut 0.5s ease-out 4.5s forwards;
    }
